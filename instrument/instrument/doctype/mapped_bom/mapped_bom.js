...
frappe.ui.form.on('Mapped BOM', {
	refresh: function(frm) {
		...
		if(frm.doc.docstatus == 1){
			...
			if(frm.doc.docstatus == 1 && frm.doc.check_propogation_to_descendent_bom && frm.doc.propogate_to_descendent_bom==0){
					frm.add_custom_button(__('Propogate Updates to Descendent BOMs'), function() {
					frappe.call({
			...
			frm.add_custom_button(__('Update References'), function() {
	...
	onload:function(frm){
		if(frm.doc.__islocal){
			...
			let name = frappe.route_options.duplicated_from_bom;
	        // frm.set_value('duplicated_from_bom', name);
			// var url = frappe.utils.get_url()
			// var query = window.location.href
			// var index = query.indexOf("=")
			// var string_data= query.substring(index+1)
			// var data1 = JSON.parse(decodeURIComponent(string_data));
			if(frm.doc.duplicated_from_bom){
				frappe.call({
					method : "instrument.instrument.custom_instrument.bom.bom.duplicate_bom",
					args : {
						doc : frm.doc.duplicated_from_bom
					},
					callback:function(r){
						if(r.message){
							var data = r.message
							...
							frm.set_value('with_operations',data.with_operations)
							if(data.items){
...
frappe.ui.form.on('Mapped BOM Item', {
	...
	item_code:function(frm,cdt,cdn){
		var row = locals[cdt][cdn]
		frappe.model.set_value(row.doctype, row.name, 'is_map_item',0)
		if(row.item_code){
			frappe.call({
				method : "instrument.instrument.doctype.mapped_bom.mapped_bom.get_default_bom",
				args:{
